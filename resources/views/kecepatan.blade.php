                ...
                <table class="table align-items-center mb-0 data-tables mt-2">
                  <thead>
                    <tr>
                      ...
                      <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">{{ __('Keterangan') }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    @foreach ($data as $kecepatanair)
                    <tr>
                      <td class="text-center px-2 text-secondary text-xxs">
                        <p class="text-xs font-weight-bold mb-0">{{ $loop->iteration }}</p>
                      </td>
                      <td class="text-center align-middle">
                        <p class="text-xs font-weight-bold mb-0">{{ $kecepatanair->formatted_waktu}}</p>
                      </td>
                      <td class="text-center align-middle">
                          <p class="text-xs font-weight-bold mb-0">{{ $kecepatanair->kecepatan}}</p>
                      </td>
                      @php
                      ...
                      @endphp
                      <td class="text-center align-middle">
                        <p class="text-xs font-weight-bold mb-0 {{ $color }}">{{ $kecepatanair->keterangan}}</p>
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
...
@include('components.modal-download', ['action' => route('kecepatan.download')])
@push('scripts')
    <script src="{{ asset('assets/js/kecepatan.js')}}"></script>
@endpush
